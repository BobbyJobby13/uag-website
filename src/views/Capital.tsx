  ...
  return (
    <div className="mx-auto max-w-6xl p-8">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Panel className="lg:col-span-2">
          ...
          <div className="space-y-3">
            {tiers.map((tier) => (
              <div
                key={tier.name}
                className="flex items-center justify-between rounded-lg bg-[#111827] p-4"
              >
                <div>
                  <div className="text-sm font-semibold text-white">{tier.name}</div>
                  <div className="text-xs text-[#8b92a8]">{tier.focus}</div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-medium text-white">{tier.range}</div>
                  <div className="text-xs text-[#5d6a87]">Allocation window</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <Panel>
          <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg bg-[#111827]">
          ...
          <button
            type="button"
            className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-500"
          >
            <DollarSign size={16} />
            Invest Now
          </button>
